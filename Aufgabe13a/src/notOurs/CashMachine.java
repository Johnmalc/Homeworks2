...
public class CashMachine {
	...
	public CashMachine(Account[] accounts) {
		CashMachine.accounts = accounts;
		CashMachine.status = Status.READY;
	}
	...
	private boolean checkPin(int pin) {
		...
		for (Account blubb : accounts) {
			if (blubb.getAccountNumber() == cashCard.accountNumber
					&& blubb.getPin() == pin) {
				return true;
			}
	...
	public void accountStatement() {
		try {
			if (status == Status.PIN_CORRECT) {
				...
				for (Account bob : accounts) {
					if (bob.getAccountNumber() == cashCard.accountNumber) {
						Geldautomat.setInfoText("Aktueller Kontostand :"
								+ bob.getBankDeposit());
						Geldautomat.setInfoText("Aktueller Dispokredit :"
								+ bob.getOverdraft());
						Geldautomat.setInfoText("Kontonummer :"
